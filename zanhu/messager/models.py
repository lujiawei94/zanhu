...
class MessageQuerySet(models.query.QuerySet):
    ...
    def get_conversation(self, sender, recipient):
        qs_one = self.objects.filter(sender=sender, recipient=recipient)
        qs_two = self.objects.filter(sender=recipient, recipient=sender)
        return qs_one.union(qs_two).order_by('created_at')

    def get_most_recent_conversation(self, recipient):
        try:
            qs_sent = self.objects.filter(sender=recipient)  # 当前登录用户发送的消息
            qs_received = self.objects.filter(recipient=recipient)  # 当前登录用户接收的消息
